...
const Approvals = () => {
  ...
  const totalPages = Math.ceil(totalRequests.length / itemsPerPage);
  ...
  const ApprovalRequest = async (id, isApproved) => {
    try {
      ...
      // Determine the updated status based on risk category
      let updatedStatus;
      if (chemicalData.riskCategory === 2) {
        updatedStatus = 1; // High risk
      } else {
        updatedStatus = 2; // Medium or Low risk
      }
      ...
      const updatedRequest = {
        ...
        supervisorApproveStatus: isApproved, // Set based on the button click
      };

      // Log the updated request object for debugging
      console.log("Sending updated request to API:", updatedRequest);

      // Update request status in the backend
      await updateRequest(updatedRequest); // Send the updated request to the API
      ...
      // Handle UI feedback based on updated status
      if (updatedStatus === 1) {
        handleShow(); // Show relevant modal for high risk
      } else if (updatedStatus === 2) {
        handleSupervisorShow(); // Show relevant modal for medium/low risk
      }

      // Optionally, you may want to fetch requests again
      fetchRequests();
    } catch (error) {
      console.error(
        ...
        <div className="col p-4">
          <div className="d-flex align-items-center justify-content-between mt-4">
            <div className="d-flex align-items-center gap-3">
              <h2 className="h4 mb-0">Approvals</h2>
            </div>
                    ...
                    <tr key={request.id}>
                      ...
                      >
                        <td
                          style={{ color: "#0d6efd" }}
                          onClick={() =>
                            handlePopoverToggle(request.chemicalId)
                          }
                        >
                          View
                        </td>
                      ...
                      <td className="text-center ">
                        <button
                          ...
                          onClick={() => ApprovalRequest(request.id, true)} // Pass true for approve
                        >
                          Approve
                        </button>
                        <button
